...
user.profile.notifications = user.profile.notifications.filter((notif) => notif.from.toString() != connection_id && notif.notifType != "pending")
user.profile.notifications.push({
  from: connection_id,
  to: user_id,
   notifType: "accepted",
...
router.post("/notif/delete", async (req, res) => {
    ...
    const user = await User.findById(user_id).populate("profile");
    user.profile.notifications = user.profile.notifications.filter((notif) => notif._id.toString() != notif_id);
    await user.profile.save();
...
router.post("/reject", async (req, res) => {
    const { user_id, connection_id } = req.body;
    const user = await User.findById(user_id).populate("profile");
    user.profile.pendingRequests = user.profile.pendingRequests.filter((pId) => pId != connection_id)
    user.profile.notifications = user.profile.notifications.filter((notif) => notif.from.toString() != connection_id && notif.notifType != "pending")
    await user.profile.save();
    res.status(200).send(user.profile)
})
