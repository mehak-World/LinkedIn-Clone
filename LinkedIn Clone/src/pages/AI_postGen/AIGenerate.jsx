...
const AIGenerate = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGeneratedPost(res.data); // Expects: {  content, images }
    } catch (err) {
  ...
  const handleApprove = async () => {
    // Save to db
   const res =  await axios.post("http://localhost:3000/posts/save", {content: generatedPost.content, image_url: generatedPost.image_url, user_id: user._id})
   console.log(res);
   navigate("/feed");
  };
  ...
  return (
    ...
    <MainNav />
     <div className="max-w-xl mx-auto p-4 my-[5%]">
      <h2 className="text-2xl font-bold mb-4">Generate LinkedIn Post with AI</h2>
      <input
        type="text"
